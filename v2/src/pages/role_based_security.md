...
NodeOPCUA ships a complete, modular, and spec-conformant implementation of **Role-Based Access Control (RBAC)** and **User Management** (OPC 10000-18) available in open source under the MIT license.
...
| [`node-opcua-role-set-test`](https://github.com/node-opcua/node-opcua/tree/master/packages/node-opcua-role-set-test) | Integration tests and a runnable sample server (private). |
...
### `WellKnownRoles` and `WellKnownRoleIds`
...
### `ClientRoleSet` and `ClientUserManagement`
...
| Criteria Type | Description and Matching Logic |
|---|---|
